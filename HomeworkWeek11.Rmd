...
The song "Genesis" by Grimes has a very steady beat throughout the track, which can be observed in the tempogram. At around 0:40 the vocals of the song start, which leads to some different tempi being picked up and displayed in the tempogram. At around 3:50, the steady beat that can be heard throughout the song stops and the song ends with airy singing and a piano melody. In the tempogram, this can be observed as the steady beat around 340 bpm becoming less clear. 
...
ggplot(Compare, aes(x = tempo)) +
    geom_bar(binwidth = 0.7, fill = "black", width = 0.7) +
    facet_wrap(~category)
```

### **Analysis**

Just like the sitribution of keys, GrimesAI also shows less varietyin tempi compared to Grimes. This could be interpreted, as AI songs being less "original" or unique than regularly produced songs. Additionally, GrimesAI songs are also a bit faster than the Grime's counterparts. There is a high occurence of songs with a tempo of around 140 bpm, while for Grime's songs the most occuring tempo is about 110 bpm.
